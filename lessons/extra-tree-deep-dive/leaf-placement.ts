...
const TRUNK_WIND_GLSL = `
  float spatialPhase = dot(vWorldPos.xz, uWindDir) * 0.5;
  float trunkPhase = spatialPhase + uTime * uTrunkSwaySpeed;
  float trunkSway = sin(trunkPhase) * (1.0 + uGustStrength);
  float heightFactor = clamp(vWorldPos.y / 3.5, 0.0, 1.0);
  heightFactor *= heightFactor;
  transformed.x += uWindDir.x * trunkSway * uWindStrength * uTrunkSwayAmount * heightFactor;
  transformed.z += uWindDir.y * trunkSway * uWindStrength * uTrunkSwayAmount * heightFactor;
  transformed.y -= abs(trunkSway * uWindStrength * uTrunkSwayAmount * heightFactor) * 0.15;
`

const BRANCH_WIND_GLSL = TRUNK_WIND_GLSL + `
  float branchPhase = spatialPhase + uTime * uBranchSwaySpeed * 1.3;
...
function createBarkMaterial(windLayer: 'trunk' | 'branch' = 'branch'): THREE.MeshStandardMaterial {
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    const windCode = windLayer === 'trunk' ? TRUNK_WIND_GLSL : BRANCH_WIND_GLSL
...
leafMat.onBeforeCompile = (shader) => {
  ...
  shader.vertexShader = shader.vertexShader.replace('#include <begin_vertex>',
    `#include <begin_vertex>
      ...
      vWorldPos = (modelMatrix * vec4(transformed, 1.0)).xyz;
    #endif
    ${params.windEnabled ? LEAF_WIND_GLSL : ''}
    `)
  shader.fragmentShader = shader.fragmentShader.replace('#include <common>',
    `#include <common>\nvarying vec3 vWorldPos;`)
...
function animate() {
  ...
  if (params.windEnabled) {
    ...
    if (params.gustEnabled) {
      ...
      windUniforms.uGustStrength.value = Math.max(0, Math.sin(gustPhase) * Math.sin(gustPhase * 0.7)) * params.gustStrength
    } else {
